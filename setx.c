    ...
    for (size_t j = 0; j < set->lines[i].n_elem; j++)
        if (hash == set->lines[i].elems[j].hash
            ...
            if (set->lines[i].n_elem-- > 1) {
                set->lines[i].elems[j].hash = set->lines[i].elems[set->lines[i].n_elem].hash;
                set->lines[i].elems[j].value = set->lines[i].elems[set->lines[i].n_elem].value;
                set->lines[i].elems[j].len = set->lines[i].elems[set->lines[i].n_elem].len;
            }
            set->n_elem--;
            return;
        }
...
int xset_add(xset_t *set, char *value, size_t len)
{
    ...
    for (ssize_t j = 0; j < set->lines[i].n_elem; j++)
        if (!strncmp(set->lines[i].elems[j].value, value, MIN(set->lines[i].elems[j].len, len)))
            return 0;
