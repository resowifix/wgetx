...
int wgetx_process_data(FILE *file, char *data, ssize_t length, uint8_t is_chunked,
        ssize_t *chunk_remaining, wgetx_data_processor_t processor, void *processor_arg)
{
    ...
    if (is_chunked) {
        if (*chunk_remaining) {
            if (length >= *chunk_remaining + 2) {
                if (data[*chunk_remaining] == '\r' && data[*chunk_remaining + 1] == '\n') {
                    ...
                    *chunk_remaining = 0;
                    return wgetx_process_data(file, data, length, is_chunked, chunk_remaining,
        ...
        *chunk_remaining = strtol(data, &cur, 16);
        ...
        cur += 2;
        return wgetx_process_data(file, cur, length - (cur - data), is_chunked, chunk_remaining,
                processor, processor_arg);
    }
...
int wgetx_parse_header_line(char **header, char **data)
{
    ...
    for (field_end = field_name; *field_end && !(field_end[0] == ':' && field_end[1] == ' ');
            field_end++) {
    ...
    for (field_end += 2; *field_end && !(field_end[0] == '\r' && field_end[1] == '\n');
            field_end++) {}
    if (!*field_end)
        return -1;
    ...
    *header = field_end;
    ...
    if (strnlen(field_name, 18) == 17 && !memcmp(field_name, "Transfer-Encoding", 18)) {
...
long wgetx_parse_http_response(char *recv_data, ssize_t rcv_len, void **data, ssize_t *data_len,
        uint8_t *is_chunked)
{
    char *cur;
    char *rep_code;
...
int wgetx_prepare_socket(wgetx_url_info_t *url_info, char *root_path, wgetx_cnx_ctx_t *ctx)
{
    ...
    if ((ret = getaddrinfo(url_info->host, url_info->is_secure ? "https" : "http", NULL, &addrinfo)
                != 0)) {
        ...
        wgetx_data_processor_t processor, void *processor_arg)
{
    ...
    if ((answer_len = SSL_read(ssl, ctx.answer, PACKET_MAX_LEN)) > 0) {
        switch ((code = wgetx_parse_http_response(ctx.answer, answer_len, &data, &data_len,
                         &is_chunked))
                / 100) {
            case 1:
            case 3:
    ...
    while (data_len > 0
            && wgetx_process_data(ctx.file, (char *)data, data_len, is_chunked, &chunk_remaining,
                       processor, processor_arg)
                       == 0) {
        ...
        wgetx_data_processor_t processor, void *processor_arg)
{
    ...
    while (data_len > 0
            && wgetx_process_data(ctx.file, data, data_len, is_chunked, &chunk_remaining, processor,
                       processor_arg)
                       == 0) {
